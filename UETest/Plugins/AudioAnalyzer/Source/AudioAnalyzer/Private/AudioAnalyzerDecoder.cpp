...
{
	_audioMetadataManager = std::make_unique<AudioAnalyzerTaglibManager>();
}
...
bool AudioAnalyzerDecoder::loadAudioInfoFromMp3(uint8* dataPtr, unsigned int dataSize)
{
	...
	config.encodingFormat = ma_encoding_format::ma_encoding_format_mp3;

	ma_result result = ma_decoder_init_memory(dataPtr, dataSize, &config, &_audioDecoder);
	if (result == MA_SUCCESS)
	{
		_audioInfo = std::make_unique<PCAudioAnalyzer::AUDIOFILE_INFO>();
		...
		result = ma_decoder_get_length_in_pcm_frames(&_audioDecoder, &totalFrameCount);
		if (result == MA_SUCCESS)
		{
			_audioInfo->dataChunkSize = totalFrameCount * ma_get_bytes_per_frame(_audioDecoder.outputFormat, _audioDecoder.outputChannels);
			_decodedPCMBuffer = std::make_unique<ma_uint8[]>(_audioInfo->dataChunkSize);
			ma_uint64 readedFrames;
			ma_decoder_read_pcm_frames(&_audioDecoder, _decodedPCMBuffer.get(), totalFrameCount, &readedFrames);
...
bool AudioAnalyzerDecoder::loadAudioInfoFromOgg(uint8* dataPtr, unsigned int dataSize)
{
	...
	ma_decoder_config config = ma_decoder_config_init_default();
	config.encodingFormat = ma_encoding_format::ma_encoding_format_vorbis;
...
bool AudioAnalyzerDecoder::loadAudioInfoFromFlac(uint8* dataPtr, unsigned int dataSize)
{
	...
	ma_decoder_config config = ma_decoder_config_init_default();
	config.encodingFormat = ma_encoding_format::ma_encoding_format_flac;
...
bool AudioAnalyzerDecoder::loadAudioInfoFromWav(uint8* dataPtr, unsigned int dataSize)
{
	...
	if (headerChunkID == "RIFF" || format == "WAVE")
	{
		...
		ma_decoder_config config = ma_decoder_config_init_default();
		config.encodingFormat = ma_encoding_format::ma_encoding_format_wav;
