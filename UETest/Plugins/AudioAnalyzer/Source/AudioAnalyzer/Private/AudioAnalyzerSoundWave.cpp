...
UAudioAnalyzerSoundWave::UAudioAnalyzerSoundWave(const FObjectInitializer& ObjectInitializer):USoundWaveProcedural(ObjectInitializer), _parentSource(nullptr)
{
}

UAudioAnalyzerSoundWave::~UAudioAnalyzerSoundWave()
{
	InvalidateSoundWave();
}
...
#if AUDIOANALYZER_ENABLE_UE5_CODE
...
int32 UAudioAnalyzerSoundWave::OnGeneratePCMAudio(TArray<uint8>& OutAudio, int32 NumSamples)
{
	...
	if (HasParent())
	{
		...
		if (audioInfoPtr)
		{
			...
			int32 validSamples = bytesToCopy / (audioInfoPtr->bitDepth / 8);
			return validSamples;
		}
	}
	return 0;
...
{
	_parentSource = nullptr;
	StopSound();
}

bool UAudioAnalyzerSoundWave::HasParent() const
{
	return (_parentSource != nullptr);
}
...
#if AUDIOANALYZER_ENABLE_UE5_CODE
TUniquePtr<Audio::IProxyData> UAudioAnalyzerSoundWave::CreateNewProxyData(const Audio::FProxyDataInitParams& InitParams)
{
	return nullptr;
}
#endif
