...
public:
	...
	float MinLoopDurationInSeconds = 0.05f;
	...
	/** Settings for a FCustomWaveProxyReader. */
	struct FSettings
	{
...
private:
	 ...
	 */
	FCustomWaveProxyReader(FSoundWaveProxyRef InWaveProxy, const FSettings& InSettings, const float InMinLoopDurationInSeconds);
...
public:
	 ...
	 * @param InMinLoopDurationInSeconds - Min loop duration in seconds. Should be set based on overlap size. 
	 */
	static TIMESTRETCHMETANODES_API TUniquePtr<FCustomWaveProxyReader> Create(FSoundWaveProxyRef InWaveProxy, const FSettings& InSettings, const float InMinLoopDurationInSeconds);

	/** Set whether the reader should loop the audio or not. */
	TIMESTRETCHMETANODES_API void SetIsLooping(bool bInIsLooping);
	...
	TIMESTRETCHMETANODES_API void SetLoopStartTime(float InLoopStartTimeInSeconds);
	 ...
	 */
	TIMESTRETCHMETANODES_API void SetLoopDuration(float InLoopDurationInSeconds);
	 ...
	 */
	TIMESTRETCHMETANODES_API bool SeekToTime(float InSeconds);
	 ...
	 * @return true on success, false on failure.
	 */
	TIMESTRETCHMETANODES_API bool SeekToFrame(uint32 InFrameNum);
