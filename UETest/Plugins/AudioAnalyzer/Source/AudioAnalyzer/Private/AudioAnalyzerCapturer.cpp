											...
											uint32 periods)
{
	...
	_audioBuffer = std::make_unique<QueueBuffer>();
...
void AudioAnalyzerCapturer::captureCallBack(void* pOutput, const void* pInput, ma_uint32 frameCount)
{
	if (_captureDeviceManager.hasDevice)
	{
		...
		if (bytesPerSample > 0)
		{
			...
			std::unique_ptr<uint8[]> tempBuffer = std::make_unique<uint8[]>(length);
...
bool AudioAnalyzerCapturer::loadAudio(uint32 periodSizeInFrames, uint32 periodSizeInMilliseconds, uint32 periods)
{
	...
	if (audioInfoPtr)
	{
		...
		ma_format capturer_audio_format = (ma_format)PCAudioAnalyzer::formatToMa_format(audioInfoPtr->audioFormat, audioInfoPtr->bitDepth);
		if (capturer_audio_format == ma_format_unknown || capturer_audio_format == ma_format_u8)
		{
			UE_LOG(LogParallelcubeAudioAnalyzer, Error, TEXT("Invalid audio format (%d,%d), only supported s16, s24, s32, f32"), audioInfoPtr->audioFormat, audioInfoPtr->bitDepth);
		}

		_captureDeviceManager.deviceConfig.capture.format = capturer_audio_format;
