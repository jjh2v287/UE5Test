...
public:
	...
	void NotifyEventWithParams(const FString& EventId, UObject* Sender, UObject* Listener, const TArray<FOutputParam, TInlineAllocator<8>>& Outparames);
	...
	void NotifyEvent(const FString& EventId, UObject* Sender, UObject* Listener, TArgs&&... Args);
...
template<typename... TArgs>
void UGIEventSubsystem::NotifyEvent(const FString& EventId, UObject* Sender, UObject* Listener, TArgs&&... Args)
{
	TArray<FOutputParam, TInlineAllocator<8>> VOutputParam = { MakeOutputParam(Args)... };

	this->NotifyEventWithParams(EventId, Sender, Listener, VOutputParam);
}
