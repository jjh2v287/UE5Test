...
UENUM(BlueprintType)
enum class ETraceProType : uint8 {
...
typedef bool (*TrType) (TArray<FHitResult>&, const FVector&, const FVector&, ECollisionChannel, const FCollisionQueryParams&, const TArray<TEnumAsByte<EObjectTypeQuery>> &, FName, ETraceProType, int32, UObject*);
...
public:
		...
		const TArray<TEnumAsByte<EObjectTypeQuery>> & ObjectTypes, FName ProfileName, ETraceProType TraceType, int32 MaxCount, UObject* WorldContextObject)
	{
		OutHits.Reset();
		UWorld* World = WorldContextObject->GetWorld();
		switch (TraceType)
		{
		case ETraceProType::TrChannel: World->LineTraceMultiByChannel(OutHits, VStart, End, TraceChannel, Params); break;
		case ETraceProType::TrObject:  World->LineTraceMultiByObjectType(OutHits, VStart, End, ObjectTypes, Params); break;
		case ETraceProType::TrProfile: World->LineTraceMultiByProfile(OutHits, VStart, End, ProfileName, Params); break;
		}
		if(OutHits.Num())
		{
			if(MaxCount > 0)
		...
		const TArray<TEnumAsByte<EObjectTypeQuery>> & ObjectTypes, FName ProfileName, ETraceProType TraceType, int32 MaxCount, UObject* WorldContextObject)
	{
		...
		switch (TraceType)
		{
		case ETraceProType::TrChannel: return  World->LineTraceSingleByChannel(OutHits[0], VStart, End, TraceChannel, Params);
		case ETraceProType::TrObject:  return  World->LineTraceSingleByObjectType(OutHits[0], VStart, End, ObjectTypes, Params);
		case ETraceProType::TrProfile: return  World->LineTraceSingleByProfile(OutHits[0], VStart, End, ProfileName, Params);
...
#if UE_VERSION_NEWER_THAN(4, 27, 99)
	inline static const TArray<TrType> TraceWork =  {&LineTraceSingle, &LineTraceMulti, &LineTraceBounce};
#endif
	...
	static FVector VectorRotate(const FVector R, const FQuat& AddRotation)
	{
		...
		return (Q*(FVector::DotProduct(Q,R)*2)) + ((FMath::Pow(AddRotation.W, 2) - FVector::DotProduct(Q,Q)) * R) + (FVector::CrossProduct(Q,R) * AddRotation.W * 2);
	}
	...
	static TArray<FHitResult> RunTrace(TArray<TArray<FVector>> Base, TrType CurrentTrace, bool Visualize, ECollisionChannel CollisionChannel, const FCollisionQueryParams& CollisionParams,
		const TArray<TEnumAsByte<EObjectTypeQuery>>& ObjectTypes, FName ProfileName, ETraceProType TraceType, int MaxCount, UObject* WorldContextObject)
	{
