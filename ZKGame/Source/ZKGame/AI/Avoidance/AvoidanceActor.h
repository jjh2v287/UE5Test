...
#include "AvoidanceActor.generated.h"

UCLASS()
class ZKGAME_API AAvoidanceActor : public AActor
{
    GENERATED_BODY()
...
public:    
    ...
    // 분리 및 회피 벡터 계산 함수들
    FVector CalculateSeparationForce(const TArray<AAvoidanceActor*>& NearbyActors);
    FVector CalculatePredictiveAvoidanceForce(const TArray<AAvoidanceActor*>& NearbyActors);
    FVector CalculateEnvironmentAvoidanceForce();
    
    // 근처 액터 탐색
    void FindNearbyActors(TArray<AAvoidanceActor*>& OutNearbyActors, float SearchRadius);
