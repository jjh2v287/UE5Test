...
FAutoConsoleVariableRef CVarCustomWaveProxyReaderSimulateSeekOnNonSeekable(
	TEXT("au.CustomWaveProxyReader.SimulateSeek"),
...
uint32 FCustomWaveProxyReader::ConformDecodeSize(uint32 InMaxDesiredDecodeSizeInFrames)
{
 ...
 * @param InMinLoopDurationInSeconds - Min loop duration in seconds. Should be set based on overlap size. 
 */
FCustomWaveProxyReader::FCustomWaveProxyReader(FSoundWaveProxyRef InWaveProxy, const FSettings& InSettings, const float InMinLoopDurationInSeconds)
	: MinLoopDurationInSeconds(InMinLoopDurationInSeconds)
 ...
 * @param InMinLoopDurationInSeconds - Min loop duration in seconds. Should be set based on overlap size. 
 */
TUniquePtr<FCustomWaveProxyReader> FCustomWaveProxyReader::Create(FSoundWaveProxyRef InWaveProxy, const FSettings& InSettings, const float InMinLoopDurationInSeconds)
{
	if (InWaveProxy->GetSampleRate() <= 0.f)
	{
		UE_LOG(LogAudio, Warning, TEXT("Cannot create FCustomWaveProxyReader due to invalid sample rate (%f). Package: %s"), InWaveProxy->GetSampleRate(), *InWaveProxy->GetPackageName().ToString());
		return TUniquePtr<FCustomWaveProxyReader>();
	}

	if (InWaveProxy->GetNumChannels() <= 0)
	{
		UE_LOG(LogAudio, Warning, TEXT("Cannot create FCustomWaveProxyReader due to invalid num channels (%d). Package: %s"), InWaveProxy->GetNumChannels(), *InWaveProxy->GetPackageName().ToString());
		return TUniquePtr<FCustomWaveProxyReader>();
	}

	if (InWaveProxy->GetNumFrames() <= 0)
	{
		UE_LOG(LogAudio, Warning, TEXT("Cannot create FCustomWaveProxyReader due to invalid num frames (%d). Package: %s"), InWaveProxy->GetNumFrames(), *InWaveProxy->GetPackageName().ToString());
		return TUniquePtr<FCustomWaveProxyReader>();
	}

	return TUniquePtr<FCustomWaveProxyReader>(new FCustomWaveProxyReader(InWaveProxy, InSettings, InMinLoopDurationInSeconds));
}


/** Set whether the reader should loop the audio or not. */
void FCustomWaveProxyReader::SetIsLooping(bool bInIsLooping)
{
...
void FCustomWaveProxyReader::SetLoopStartTime(float InLoopStartTimeInSeconds)
{
...
void FCustomWaveProxyReader::SetLoopDuration(float InLoopDurationInSeconds)
{
...
bool FCustomWaveProxyReader::SeekToTime(float InSeconds)
{
...
bool FCustomWaveProxyReader::CanProduceMoreAudio() const
{
...
bool FCustomWaveProxyReader::SeekToFrame(uint32 InFrameNum)
{
	// ignore seek request if we're already at the specified time
 ...
 */
int32 FCustomWaveProxyReader::PopAudio(Audio::FAlignedFloatBuffer& OutBuffer)
{
	using namespace Audio;

	TRACE_CPUPROFILER_EVENT_SCOPE(FCustomWaveProxyReader::PopAudio);
	...
	if (OutBufferView.Num() > 0)
	{
		...
		if (Settings.bMaintainAudioSync)
		{
			...
			// if we were asked to maintain audio sync, then do some extra bookkeeping
...
int32 FCustomWaveProxyReader::PopAudioFromDecoderOutput(TArrayView<float> OutBufferView)
{
	check(NumChannels > 0);
...
bool FCustomWaveProxyReader::InitializeDecoder(float InStartTimeInSeconds)
{
	using namespace Audio;

	TRACE_CPUPROFILER_EVENT_SCOPE(FCustomWaveProxyReader::InitializeDecoder);
...
int32 FCustomWaveProxyReader::DiscardSamples(int32 InNumSamplesToDiscard)
{
...
float FCustomWaveProxyReader::ClampLoopStartTime(float InStartTimeInSeconds)
{
	return FMath::Clamp(InStartTimeInSeconds, 0.f, MaxLoopStartTimeInSeconds);
}

float FCustomWaveProxyReader::ClampLoopDuration(float InDurationInSeconds)
{
...
void FCustomWaveProxyReader::UpdateLoopBoundaries()
{
	if (Settings.bIsLooping)
	{
...
FCustomWaveProxyReader::EDecodeResult FCustomWaveProxyReader::Decode()
{
