...
{
	/** Copy from Epic Implementation then revise to allow random access to current data instead of always starting at ReadIndex
	 * ------------
	...
	using FMultichannelCircularBufferRand = TArray<TCircularAudioBufferCustom<float>>;

	TIMESTRETCHMETANODES_API void SetMultichannelCircularBufferCapacity(int32 InNumChannels, int32 InNumFrames, FMultichannelCircularBufferRand& OutBuffer);

	TIMESTRETCHMETANODES_API int32 GetMultichannelBufferNumFrames(const FMultichannelCircularBufferRand& InBuffer);

	TIMESTRETCHMETANODES_API int32 GetMultichannelBufferNumFrames(const Audio::FMultichannelBuffer& InBuffer);

	TIMESTRETCHMETANODES_API int32 GetMultichannelBufferNumFrames(const Audio::FMultichannelBufferView& InBuffer);
