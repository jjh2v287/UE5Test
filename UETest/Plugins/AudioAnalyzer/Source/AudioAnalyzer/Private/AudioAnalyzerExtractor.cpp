...
UAudioAnalyzerSoundWave* AudioAnalyzerExtractor::getAudioComponentSoundWave() const
{
#if AUDIOANALYZER_ENABLE_UE5_CODE
	return dynamic_cast<UAudioAnalyzerSoundWave*>(_audioComponent->Sound.Get());
#else
	return dynamic_cast<UAudioAnalyzerSoundWave*>(_audioComponent->Sound);
#endif
}
...
void AudioAnalyzerExtractor::playAudioComponent(int32 loops, float startTime)
{
	if (_audioComponent && _audioComponent->IsValidLowLevel())
	{
		UAudioAnalyzerSoundWave* soundWave = getAudioComponentSoundWave();
		if (soundWave)
		{
