...
public:
	...
	// @param bOutIsInherited True if the selected key is inherited from a parent blackboard asset
	// @return A pointer to the currently selected entry (nullptr if nothing is selected or a category)
...
protected:
	...
	TSharedRef<SWidget> HandleCreateWidgetForAction(struct FCreateWidgetForActionData* const InCreateData);
	...
	virtual void FillContextMenu(class FMenuBuilder& MenuBuilder) const;

	// Extend the toolbar
	virtual TSharedPtr<class FExtender> GetToolbarExtender(TSharedRef<FUICommandList> ToolkitCommands) const;
