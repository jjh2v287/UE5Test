...
{
	NodeTooltip = LOCTEXT("NodeTooltip", "Generic sort array by property based on quicksort algorithm.");
}
...
void UK2Node_ArraySort::AllocateDefaultPins()
{
	...
	SetPinToolTip(*TargetArrayPin, LOCTEXT("TargetArrayPinDescription", "The TargetArray to set new row"));
	...
	SetPinToolTip(*AscendingPin, LOCTEXT("AscendingPinDescription", "The returned TableRow, if found"));
...
{
	return LOCTEXT("NodeCategory", "Utilities|Array");
}
...
{
	return LOCTEXT("ListViewTitle", "Array Sort By Property");
}

FText UK2Node_ArraySort::GetKeywords() const
{
	return LOCTEXT("NodeKeywords", "Array,Sort,Wildcard,k2node");
}
...
void UK2Node_ArraySort::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	const FName DstFunctionName = GET_FUNCTION_NAME_CHECKED(UBlueprintProLibrary, Array_SortV2);
	UK2Node_CallFunction* DstArraySortFunction = CompilerContext.SpawnIntermediateNode<UK2Node_CallFunction>(this, SourceGraph);
	DstArraySortFunction->FunctionReference.SetExternalMember(DstFunctionName, UBlueprintProLibrary::StaticClass());
