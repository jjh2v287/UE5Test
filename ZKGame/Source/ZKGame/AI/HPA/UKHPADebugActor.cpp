...
void AUKHPADebugActor::UpdatePathVisualization()
{
    ...
    // 서브시스템 참조 가져오기
    UUKHPAManager::Get()->AllRegisterWaypoint();

    // 시작점과 끝점에서 가장 가까운 웨이포인트 찾기
    AUKWayPoint* StartWaypoint = UUKHPAManager::Get()->FindNearestWaypoint(GetActorLocation());
    AUKWayPoint* EndWaypoint = UUKHPAManager::Get()->FindNearestWaypoint(EndActor->GetActorLocation());
    ...
    // 경로 찾기
    TArray<AUKWayPoint*> Path = UUKHPAManager::Get()->FindPath(StartWaypoint, EndWaypoint);
